...
const getDocentes = async (req = request, res = response) => {
    ...
    try {
        ...
        if (!docente){
            res.status(404).json({msg: "No se encontraron registros de los Docentes"})
            return
        }         
        res.json({docente}) 
    } catch (error) {
...
const getDocentesBYID = async (req = request, res = response) => {
    ...
    try {
        ...
        if (!maestro){
            res.status(404).json({msg: `No se encontro el docente registrado con el ID=${id}`})
            return
        }         
        res.json({maestro}) 
    } catch (error) {
...
const deleteDocentesByID = async (req = request, res = response) => {
    ...
    try {
        ...
        if (useraffectedRows === 0){
            res.status(404).json({msg: `No se pudo eliminar el Docente con el ID ${id}`})
            return
        }    
        res.json({msg: `Se elimino el docente con el ID ${id}`}) 
    } catch (error) {
        console.log(error)
        res.status(500).json({error})
    } finally{
...
if(
    ...
    try {
        ...
        //consle.log(userDelete)
        if (affectedRows === 0){
            res.status(404).json({msg: `No se pudo agregar el registro del Docente ${Nombre}`})
            return
        }    
