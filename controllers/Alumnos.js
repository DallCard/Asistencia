...
const modeloAlumnos = require ("../models/Alumnos");
...
const getAlumnos = async (req = request, res = response) => {
    ...
    try {
        ...
        if (!alumno){
            res.status(404).json({msg: "No se encontraron registros de los alumnos"})
            return
        }         
        res.json({alumno}) 
    } catch (error) {
...
const getAlumnosByID = async (req = request, res = response) => {
    ...
    try {
        ...
        if (!alumnos){
            res.status(404).json({msg: `No se encontro alumnos registrados con el ID=${id}`})
            return
        }         
        res.json({alumnos}) 
    } catch (error) {
...
const deleteAlumnosByID = async (req = request, res = response) => {
    ...
    try {
        ...
        if (useraffectedRows === 0){
            res.status(404).json({msg: `No se pudo eliminar el Alumno con el ID ${id}`})
            return
        }    
        res.json({msg: `Se elimino el alumno con el ID ${id}`}) 
    } catch (error) {
        console.log(error)
        res.status(500).json({error})
    } finally{
